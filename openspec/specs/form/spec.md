...
## Purpose
The endorsed Reltio `Form` surface: the SAP Fiori Form layout floorplan wrapped as a thin Reltio component that adds native-form submission with JSON serialization (`onSubmit`), opinionated single-column/labels-on-top layout defaults, and ref forwarding to the native `<form>`. `FormGroup` and `FormItem` remain 1:1 UI5 re-exports.
...
`FormGroup` and `FormItem` SHALL remain documentation-only 1:1 re-exports of the corresponding UI5 components.
