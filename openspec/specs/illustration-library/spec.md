# illustration-library Specification

## Purpose

Full-color, theme-aware illustration assets for empty states, errors, onboarding, and other empathetic UI moments. Distributed exclusively as tree-shakable React components from `@reltio/design/illustrations`, with three size variants (Spot 128×128, Dialog 240×160, Scene 320×220) and built-in light/dark theming through the platform's existing SAP `--sapContent_Illustrative_*` token cascade — no extra dark assets, no React state.

Illustrations are sourced verbatim from the upstream [`SAP/ui5-webcomponents`](https://github.com/SAP/ui5-webcomponents/tree/main/packages/fiori/src/illustrations) repository (Apache 2.0). A single command — `npm run build-illustrations` — downloads a curated subset, validates the inventory, and generates all derived files. Raw SVGs are private build artifacts kept under `illustrations/_source/` and are NOT served as a public CDN URL because their fills reference SAP variables that resolve only when the SVG is inlined into a host DOM that loads `public/variables.css`.
