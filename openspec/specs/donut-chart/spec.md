...
### Requirement: Center label on hover
The `DonutChart` SHALL display the segment name and percentage in the center of the donut ring when a segment is hovered. The label SHALL use the standard body font size (14px, normal weight).

#### Scenario: Hover shows center label
- **WHEN** user hovers over a segment with `name="Matched"` representing 68% of total
- **THEN** the center of the donut displays "Matched" and "68%" in 14px normal weight text
