...
### Requirement: Region labels
The `GeoChart` SHALL accept an optional `label` prop (boolean, default `false`). When `true`, region names from the GeoJSON SHALL be rendered as text labels on the map.

#### Scenario: Labels hidden by default
- **WHEN** `GeoChart` renders without a `label` prop
- **THEN** no region name labels are shown on the map

#### Scenario: Labels shown
- **WHEN** `GeoChart` renders with `label={true}`
- **THEN** each region displays its name as a text label

### Requirement: Tooltip with units
The `GeoChart` SHALL display a tooltip on region hover showing the region name and value. When the `units` prop is provided, the value SHALL include the unit suffix.
...
The `GeoChart` SHALL accept an optional `height` prop (number or string) passed through to the base `Chart` component. The default height SHALL be 300px.

#### Scenario: Default height
- **WHEN** `GeoChart` renders without a `height` prop
