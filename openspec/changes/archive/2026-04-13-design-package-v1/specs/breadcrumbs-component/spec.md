...
The Breadcrumbs component is a navigation breadcrumb trail showing page hierarchy. It uses `<nav>` + `<ol>` for proper [semantic markup](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/nav). Two components: Breadcrumbs (container) and Breadcrumb (individual item). Breadcrumb is a private subcomponent within the Breadcrumbs folder. It follows all Reltio Design Platform constitution principles including CSS Modules, classNames utility, TypeScript strict typing, and accessibility standards.
