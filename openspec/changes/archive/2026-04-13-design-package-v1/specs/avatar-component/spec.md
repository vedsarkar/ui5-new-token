...
The Avatar component displays a user representation with three fallback levels: image, initials, and a generic icon. It renders a `<span>` root element containing either an `<img>`, text content, or a default person icon. Used in Profile (L3), AppBar (L2), and ActivityLog (L4). Avatar is non-interactive by default — clickability is added by wrapping it in a button or link at the consumer level.
