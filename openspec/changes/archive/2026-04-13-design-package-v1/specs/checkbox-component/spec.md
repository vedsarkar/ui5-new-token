...
The Checkbox component is a controlled selection control that supports checked, unchecked, and indeterminate visual states. It wraps a native `<input type="checkbox">` inside a `<label>` element with custom visual styling following Material Design 3, while preserving full native behavior for form submission, keyboard interaction, and accessibility.

The CheckboxGroup component is a layout container for Checkbox components, providing shared disabled/error state and vertical or horizontal orientation.

Both components follow all Reltio Design Platform constitution principles including CSS Modules, `classNames()` utility, `--reltio-color-*` tokens, TypeScript strict typing, and accessibility standards.
