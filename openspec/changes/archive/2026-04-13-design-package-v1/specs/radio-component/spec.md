...
The Radio component is a selection control for choosing one option from a group. It wraps a native `<input type="radio">` inside a `<label>` with custom visual styling while preserving full keyboard and screen reader accessibility. The companion RadioGroup component manages mutual exclusion — only one Radio in a group can be selected at a time. RadioGroup is controlled-only (`value` + `onChange`).

Default visual style follows Material Design 3 radio buttons. SAP Fiori appearance is achievable via CSS overrides on stable classes.
