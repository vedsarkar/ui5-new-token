# UserMessage Component Specification

## Purpose

The UserMessage component displays user-authored messages in a chat interface with Markdown formatting support. It provides consistent styling and layout for user messages, distinguishing them visually from assistant messages.
...
### Component Structure
- `UserMessage.tsx` - Component implementation
- `UserMessage.types.ts` - TypeScript type definitions (using `type`, not `interface`)
- `UserMessage.module.css` - CSS Modules styles with all CSS variables on .root
- `UserMessage.stories.tsx` - Storybook stories (one variant per story)
