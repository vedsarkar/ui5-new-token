...
- Generated CSS lives at `public/variables.css` — it carries **only the tokens Reltio re-values** (the delta against the stock SAP Horizon theme), under `[data-theme="sap-reltio-light"]` and `[data-theme="sap-reltio-dark"]` blocks (each block also lists the legacy `horizon-*` alias and a `:root[data-theme]` form). The full stock token set is injected at runtime by the UI5 web components (a `:root { … }` constructable stylesheet), so there is no point duplicating it. `build-tokens` computes the delta by diffing the token files against UI5's injected bundles. Auto-generated — do NOT edit manually, run `npm run build-tokens`.
