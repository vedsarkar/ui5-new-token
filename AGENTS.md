...
1. The consumer loads `https://reltio.design/variables.css`, `https://reltio.design/fonts.css` and `https://reltio.design/global.css` in `<head>`, in that order (or self-hosts them). `global.css` must come last — it corrects components on top of the token layer.
