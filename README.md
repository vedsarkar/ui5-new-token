...
- 🎨 **SAP Horizon design tokens** — generated into static `public/variables.css`, `public/fonts.css`, `public/fonts/*.woff2`, plus the hand-maintained `public/global.css` component corrections; consumed via the `data-theme` attribute
...
│   https://reltio.design/global.css                           │
...
├── packages/           # Published @reltio/* packages (auth, design)
├── apps/               # Reltio product apps — platform-owned context files (see below)
├── public/             # Static assets — variables.css, fonts.css, global.css, fonts/, images
...
The `apps/` directory holds **platform-owned context files** for Reltio product applications — story MDX, migration status, review checklists, PRDs — managed by the UI CoE.
...
└── ...                    # Other platform-owned documentation (PRDs, notes)
```

Application **source code is no longer tracked here**. Each app lives in its own Bitbucket repository and is owned by its own team. Earlier versions of this repo wired the apps in as git submodules — that approach was retired because it caused frequent friction (stale pointers, accidental commits into the platform repo, slow clones).
...
To work on the platform and one or more apps at the same time, use a **VSCode multi-root workspace**:

1. Clone each app you need into its own folder (anywhere on disk).
2. Create a `<name>.code-workspace` file that lists `reltio-design` and the app folders.
3. Open the workspace in VSCode/Cursor — every folder behaves as an independent git repo with its own status, branches, and commits.

If you prefer to keep everything under one tree, you can clone an app into `apps/<name>/src/` locally. The `apps/*/src/` pattern in `.gitignore` keeps those checkouts untracked, so they will never be committed back into reltio-design.

### Read-only contract

> **All changes to application repositories are made through the standard git workflow in their own separate repositories.** Files outside `apps/<name>/` (or anything inside an `apps/<name>/src/` checkout) are never the right place to land app changes — work in the app's own repo and PR.

Specifically:
- **Do not** modify files inside `apps/*/src/` from reltio-design — those are local checkouts of other repos.
- **Do** add platform-owned documentation and context files to `apps/<name>/` (outside `src/`).
- **Do** use VSCode multi-root workspaces (or independent clones) to coordinate cross-repo work.
