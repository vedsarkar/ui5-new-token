...
├── apps/               # Read-only git submodules of Reltio product apps (see below)
...
The `apps/` directory contains **read-only** git submodules of Reltio product applications. They provide source-code context for AI agents and cross-application analysis (e.g. MUI to SAP UI5 migration, `@reltio/auth` adoption) — they are **not part of the platform build**.
...
> **All changes to application repositories are made through the standard git workflow in their own separate repositories.** The submodules in `apps/` are strictly read-only mirrors — never commit or push changes through them. Each application is owned and developed independently by its own team; the submodule reference in reltio-design simply provides AI agents and platform developers with a local copy of the source code for analysis.

Specifically:
- **Do not** run `git commit` or `git push` inside `apps/*` directories
- **Do not** modify files inside `apps/*` from reltio-design
