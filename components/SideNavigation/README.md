...
Set `defaultCollapsed` to start the menu collapsed on mount; the built-in toggle owns state from that point on. It only has effect together with `collapsable` — a non-collapsable menu has no way to change state and always renders expanded, so `defaultCollapsed` is ignored.
