 ...
 * `accessibleMode`. Native `id`, `className`, and `aria-*` attributes apply to
 * the outer `<form>`, not the UI5 Form. See the component README for the
 * divergence rationale.
 */
export type FormProps = Omit<
	ComponentPropsWithoutRef<typeof Form>,
	"onSubmit" | "layout" | "labelSpan" | "id" | "className"
> &
	AriaAttributes & {
		/**
		 * Native form element id. Applied to the outer `<form>`, not the UI5 Form.
		 */
		id?: string;

		/**
		 * CSS class names merged onto the outer native `<form>` wrapper.
		 */
		className?: string;
