...
### Native form identity and ARIA

Pass `id`, `className`, and standard `aria-*` attributes (`aria-label`,
`aria-labelledby`, `aria-describedby`, …) to name or style the **outer native
`<form>`** — the element that owns submission and receives the forwarded `ref`.
These are not forwarded to the inner UI5 Form. UI5 accessibility props such as
`accessibleMode` and `headerText` still apply only to the layout floorplan.
