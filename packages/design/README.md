...
3. **Curated surface.** Anything re-exported from `@reltio/design` is endorsed for use in Reltio MDM apps, with stories that cover the edge cases SAP did not document. Anything not re-exported is not yet endorsed — open an issue with the CoE to request endorsement.
